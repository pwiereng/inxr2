...
# VERIFY: Get all files the API knows about at that commit
docker exec inxr2-dev bash -c "curl -s 'http://localhost:8000/api/repositories/<repo>/files?commit=$HEAD_HASH&limit=10000' | python3 -c \"import sys,json; data=json.load(sys.stdin); [print(f['path']) for f in data.get('files', data) if isinstance(f, dict)]\"" | sort > /tmp/api_files.txt
...
docker exec inxr2-dev bash -c "curl -s 'http://localhost:8000/api/repositories/inxr2/files?commit=$HEAD_HASH&limit=10000' | python3 -c \"import sys,json; data=json.load(sys.stdin); [print(f['path']) for f in data.get('files', data) if isinstance(f, dict)]\"" | sort > /tmp/api_files.txt
comm -23 /tmp/git_files.txt /tmp/api_files.txt
```
