...
class CommitModel(Base):
    ...
    parent_hashes: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)  # Use JSON for SQLite compatibility
    ...
    author_date: Mapped[datetime] = mapped_column(DateTime(timezone=False), nullable=False)
    commit_date: Mapped[datetime] = mapped_column(DateTime(timezone=False), nullable=False)
    message: Mapped[str] = mapped_column(Text, nullable=False)
    indexed_at: Mapped[datetime] = mapped_column(DateTime(timezone=False), nullable=False, server_default="now()")

    # Relationships
    repository: Mapped["RepositoryModel"] = relationship("RepositoryModel", back_populates="commits")
    files: Mapped[list["FileModel"]] = relationship("FileModel", back_populates="commit", cascade="all, delete-orphan")
    symbols: Mapped[list["SymbolModel"]] = relationship("SymbolModel", back_populates="commit", cascade="all, delete-orphan")
    references: Mapped[list["ReferenceModel"]] = relationship("ReferenceModel", back_populates="commit", cascade="all, delete-orphan")
