...
class TestSQLiteUrlNormalization:
    """Tests for SQLite URL normalization in DatabaseConnection."""

    def test_converts_sqlite_file_url(self) -> None:
        """DatabaseConnection should convert sqlite:/// file URLs."""
        url = "sqlite:///path/to/db.sqlite"
        conn = DatabaseConnection(url)
        assert conn.database_url == "sqlite+aiosqlite:///path/to/db.sqlite"

    def test_converts_sqlite_memory_url(self) -> None:
        """DatabaseConnection should convert sqlite:///:memory: URLs."""
        url = "sqlite:///:memory:"
        conn = DatabaseConnection(url)
        assert conn.database_url == "sqlite+aiosqlite:///:memory:"

    def test_converts_sqlite_empty_url(self) -> None:
        """DatabaseConnection should convert sqlite:// URLs (in-memory variant)."""
        url = "sqlite://"
        conn = DatabaseConnection(url)
        assert conn.database_url == "sqlite+aiosqlite://"

    def test_preserves_aiosqlite_url(self) -> None:
        """DatabaseConnection should preserve sqlite+aiosqlite:// URLs."""
        url = "sqlite+aiosqlite:///path/to/db.sqlite"
        conn = DatabaseConnection(url)
        assert conn.database_url == url

    def test_sqlite_creates_engine_without_pool_settings(self) -> None:
        """DatabaseConnection with SQLite should not use PostgreSQL pool settings."""
        url = "sqlite+aiosqlite:///:memory:"
        # This would fail if pool_size/max_overflow were passed to SQLite
        conn = DatabaseConnection(url)
        assert isinstance(conn.engine, AsyncEngine)

    def test_get_database_url_converts_sqlite(self) -> None:
        """get_database_url should convert sqlite:// URLs."""
        with patch.dict(os.environ, {"DATABASE_URL": "sqlite:///test.db"}):
            result = get_database_url()
            assert result == "sqlite+aiosqlite:///test.db"

    def test_get_database_url_preserves_aiosqlite(self) -> None:
        """get_database_url should preserve sqlite+aiosqlite:// URLs."""
