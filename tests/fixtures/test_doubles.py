    ...
    async def search_by_text(
        ...
        if repository_id is not None:
            ...
            if commit_id is not None:
                ...
                # Fail fast if no file_repo is wired — otherwise refs would
                # silently fall through unscoped, masking incorrect test wiring.
                assert self._file_repo is not None, (
                    "commit_id filter requires a file_repo to be set on "
    ...
    async def save(self, status: IndexStatus) -> IndexStatus:
        ...
        if status.id is None:
            status = IndexStatus(
                ...
                last_indexing_duration_seconds=status.last_indexing_duration_seconds,
                last_resolving_duration_seconds=status.last_resolving_duration_seconds,
